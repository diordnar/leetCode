...
private:
    bool canFinish(const vector<int> &weights, int D, int cap)
    {
        int i = 0;
...
class Solution2 // LeetCode他人的解决方案
{
public:
    int shipWithinDays(vector<int> &weights, int D)
    {
    ...
    bool isok(vector<int> &weights, int w, int D)
    {
        int remain = w;
        int d = 1;
        for (int i = 0; i < weights.size(); i++)
...
int main()
{
    ...
    int D = 5;
    Solution slv;
    cout << slv.shipWithinDays(weights, D) << endl;
    return 0;
}