...
// 首先状态 dp 一定能自己想出来。
// dp[i][j] 表示 s 的前 iii 个是否能被 p 的前 jjj 个匹配
// 转移方程
...
// 只能让前一个字符消失，也就是匹配 000 次前一个字符。

// 所以按照 p[j-1] 和 s[i] 是否相等，我们分为两种情况：
// 3.1 p[j-1] != s[i] : dp[i][j] = dp[i][j-2]
//     这就是刚才说的那种前一个字符匹配不上的情况。
//     比如(ab, abc * )。遇到 * 往前看两个，发现前面 s[i] 的 ab 对 p[j-2] 的 ab 能匹配，虽然后面是 c*，
// 但是可以看做匹配 000 次 c，相当于直接去掉 c *，所以也是 True。注意 (ab, abc**) 是 False。
...
// 这三种情况是 ororor 的关系，满足任意一种都可以匹配上，同时是最难以理解的地方：
...
// dp[i][j-2] 就是 去掉多余的 b *，p 本身之前的能否匹配，###b 和 ### 是否匹配，比如 qqb qqbb*
// 之前的 qqb qqb 就可以匹配，那多了的 b*也无所谓，因为b*可以是匹配 000 次 b，相当于 b * 可以直接去掉了。
...
int main()
{
    ...
    string p = "abc..";
    Solution slv;
    cout << boolalpha << slv.isMatch(s, p) << endl;
    return 0;
}